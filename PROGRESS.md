...
**2026-07-09 — Fix: step 5 (confirm) rendered totally blank on new entries, no Save button reachable (released)**
- **Symptom reported:** real end-user (iPhone Safari) said tapping through the entry form "can't save, goes to a blank page." Screenshot from the reporter confirmed it: step-dots showing step 5 active, but the whole content area was empty — no confirm card, no Save button, nothing but the header and the back arrow.
- **Investigated the backend first:** Supabase `api` logs + the `transactions` table showed every save attempt from that user's session that day returned `201`/`204` and the rows were actually written (no duplicates, so not retry-spam either) — ruled out the write path, `increment_usage` RPC, and the `budgets_monthly` migration (already applied, despite `list_migrations` showing empty).
- **Root cause:** the 2026-07-08 sticky-footer refactor changed step 4's Continue button (now in the page-level footer) to call `advance()` directly instead of `StepDetails`'s own `onNext(note, date)`. `StepDetails` picks `today` as its local default date, but only reports that value up to the parent's `draft` via `onChange` — which only fires if the user actually edits the note/date field. Since "today" is already correct for most entries, nobody touches it, so `draft.date` stayed `undefined`. Step 5 gates *both* the confirm card and the footer's Save button behind `isDraftComplete` (`type && amount && categoryId && date`) — with `date` missing, neither rendered. No error, no fallback, just blank; the only way out was the header's back arrow. Edit-mode was never affected (`prefillFrom` always sets a real `date` from the existing row), which is why this only showed up on fresh entries and slipped past the edit-mode test coverage.
- **Fix:** default `draft.date` to `todayISO()` in the initial `useState` and in `resetToNewEntry()` ([src/app/form/page.tsx](src/app/form/page.tsx)), so the parent's default always matches what `StepDetails` displays — `isDraftComplete` no longer depends on the user having touched a field that already looked filled in.
- **Regression test:** [tests/app/form-new-entry.test.tsx](tests/app/form-new-entry.test.tsx) walks a brand-new entry through all 5 steps via footer taps only (never touching the date field) and asserts the confirm card + Save button actually render — verified it fails on the pre-fix code (times out on step 5) and passes after.
- Also added `src/app/error.tsx` + `src/app/global-error.tsx` (route error boundaries) while investigating — the app had none, so *any* future uncaught render exception would have produced this same "blank page with no way out" symptom for an unrelated reason. Now it shows a retry/home screen and logs the error instead.
...
- [ ] Verify the sticky footer + existing `BottomNav` combined *visual* footprint on production, on a short real phone (iPhone SE class) with the keyboard open on the amount/details steps — separate from the 2026-07-09 blank-step-5 bug (that one was a data/state bug, not a layout one; still shipped straight to `master` without device testing, so the layout question is still open)
